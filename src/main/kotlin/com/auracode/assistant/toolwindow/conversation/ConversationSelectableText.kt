...
internal fun TimelineTextInteractionHost(
    ...
    CompositionLocalProvider(
        ...
        LocalContextMenuRepresentation provides rememberConversationContextMenuRepresentation(palette),
    ) {
        content()
    }
}
...
@Composable
internal fun ConversationSelectableText(
...
@Composable
internal fun rememberConversationMarkdownSelectionColors(
    palette: DesignPalette,
): TextSelectionColors {
    val background = conversationMarkdownSelectionBackground(palette)
    val handle = conversationMarkdownSelectionHandle(palette)
...
internal fun rememberTimelineCommandSelectionColors(): TextSelectionColors {
    val background = conversationCommandSelectionBackground()
    val handle = conversationCommandSelectionHandle()
    return remember(background, handle) {
        TextSelectionColors(
            handleColor = handle,
            backgroundColor = background,
...
internal fun conversationMarkdownSelectionBackground(
    palette: DesignPalette,
): Color {
...
internal fun conversationMarkdownSelectionHandle(
    palette: DesignPalette,
): Color {
    return if (palette.timelineCardBg.luminance() > 0.5f) {
        palette.accent
...
internal fun conversationCommandSelectionBackground(): Color {
...
internal fun conversationCommandSelectionHandle(): Color {
...
private fun rememberTimelineCopyOnlyTextContextMenu(): TextContextMenu {
    val copyLabel = conversationCopyMenuLabel()
    return remember(copyLabel) {
        object : TextContextMenu {
            ...
            override fun Area(
                ...
                val items = conversationTextContextMenuItems(
                    copyLabel = copyLabel,
 ...
 */
internal fun conversationCopyMenuLabel(): String {
    return AuraCodeBundle.message("timeline.copy")
}

@OptIn(ExperimentalFoundationApi::class)
internal fun conversationTextContextMenuItems(
    copyLabel: String,
    onCopy: (() -> Unit)?,
