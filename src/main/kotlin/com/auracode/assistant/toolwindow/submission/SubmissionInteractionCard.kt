...
internal enum class SubmissionCardActionEmphasisStyle {
    PRIMARY_FILL,
    SUBTLE_HIGHLIGHT,
}

internal data class SubmissionCardActionChrome(
...
internal enum class SubmissionInteractionFocusTarget {
    CARD,
    INPUT,
}

internal fun restoreSubmissionInteractionFocusTarget(preferInput: Boolean): SubmissionInteractionFocusTarget {
    return if (preferInput) SubmissionInteractionFocusTarget.INPUT else SubmissionInteractionFocusTarget.CARD
}

internal fun restoreSubmissionInteractionFocus(
    target: SubmissionInteractionFocusTarget,
    cardFocusRequester: FocusRequester,
    inputFocusRequester: FocusRequester? = null,
) {
    when (target) {
        SubmissionInteractionFocusTarget.CARD -> cardFocusRequester.requestFocus()
        SubmissionInteractionFocusTarget.INPUT -> (inputFocusRequester ?: cardFocusRequester).requestFocus()
    }
}

@Composable
internal fun SubmissionInteractionCard(
    p: DesignPalette,
...
@Composable
internal fun SubmissionCardPrimaryAction(
    label: String,
    p: DesignPalette,
    modifier: Modifier = Modifier,
...
@Composable
internal fun SubmissionCardAction(
    label: String,
    emphasized: Boolean,
    p: DesignPalette,
    modifier: Modifier = Modifier,
    ...
    emphasisStyle: SubmissionCardActionEmphasisStyle = SubmissionCardActionEmphasisStyle.PRIMARY_FILL,
    enabled: Boolean = true,
    ...
    val chrome = submissionCardActionChrome(
...
internal fun submissionCardActionChrome(
    emphasized: Boolean,
    emphasisStyle: SubmissionCardActionEmphasisStyle,
    enabled: Boolean,
    danger: Boolean,
    p: DesignPalette,
): SubmissionCardActionChrome {
    if (!enabled) {
        return SubmissionCardActionChrome(
    ...
    if (emphasized && emphasisStyle == SubmissionCardActionEmphasisStyle.SUBTLE_HIGHLIGHT) {
        return SubmissionCardActionChrome(
...
private fun keyboardHintTint(
    ...
    return when {
        !enabled -> p.textMuted
        emphasized && emphasisStyle == SubmissionCardActionEmphasisStyle.SUBTLE_HIGHLIGHT -> p.accent.copy(alpha = 0.88f)
