...
import com.auracode.assistant.session.kernel.SessionSubagentStatus as DomainSessionSubagentStatus
...
/**
 * Maps domain-level subagent snapshots into composer UI models.
 */
internal fun SessionSubagentSnapshot.toSessionSubagentUiModel(): SessionSubagentUiModel {
    return SessionSubagentUiModel(
        threadId = threadId,
        displayName = displayName,
        mentionSlug = mentionSlug,
        status = when (status) {
            DomainSessionSubagentStatus.ACTIVE -> SessionSubagentStatus.ACTIVE
            DomainSessionSubagentStatus.IDLE -> SessionSubagentStatus.IDLE
            DomainSessionSubagentStatus.PENDING -> SessionSubagentStatus.PENDING
            DomainSessionSubagentStatus.FAILED -> SessionSubagentStatus.FAILED
            DomainSessionSubagentStatus.COMPLETED -> SessionSubagentStatus.COMPLETED
            DomainSessionSubagentStatus.UNKNOWN -> SessionSubagentStatus.UNKNOWN
        },
