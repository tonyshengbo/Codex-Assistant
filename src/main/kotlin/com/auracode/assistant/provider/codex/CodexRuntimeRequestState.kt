...
internal data class ActiveRequest(
    ...
    fun cancel() {
        ...
        pendingToolUserInputs.values.forEach { pending ->
            if (!pending.response.isCompleted) {
                pending.response.complete(ProviderToolUserInputSubmission(emptyMap()))
            }
        }
    ...
    fun submitToolUserInput(
        ...
        return pending.response.complete(ProviderToolUserInputSubmission(answers))
    }
}
...
internal data class PendingToolUserInput(
    ...
    val response: CompletableDeferred<ProviderToolUserInputSubmission> = CompletableDeferred(),
