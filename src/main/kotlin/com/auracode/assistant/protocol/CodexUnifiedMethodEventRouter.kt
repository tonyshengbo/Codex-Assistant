...
internal class CodexUnifiedMethodEventRouter {
    private val state = CodexUnifiedParserState()
    private val itemTypeParsers = CodexUnifiedItemTypeParsers()
    private val itemLifecycleParser = CodexUnifiedItemLifecycleParser(itemTypeParsers)
    private val deltaEventParser = CodexUnifiedDeltaEventParser()

    fun parse(obj: JsonObject): UnifiedEvent? {
     ...
     */
    private fun parseThreadStarted(params: JsonObject): UnifiedEvent? {
        val threadId = params.objectValue("thread")?.string("id") ?: return null
        state.activeThreadId = threadId
        return UnifiedEvent.ThreadStarted(threadId = threadId)
    }
     ...
     */
    private fun parseTurnStarted(params: JsonObject): UnifiedEvent? {
        ...
        return UnifiedEvent.TurnStarted(turnId = turnId, threadId = threadId)
    }
     ...
     */
    private fun parseTurnCompleted(params: JsonObject): UnifiedEvent? {
        ...
        return UnifiedEvent.TurnCompleted(turnId = turnId, outcome = outcome, usage = usage)
    }
     ...
     */
    private fun parseThreadTokenUsageUpdated(params: JsonObject): UnifiedEvent? {
        ...
        state.activeThreadId = threadId
        return UnifiedEvent.ThreadTokenUsageUpdated(
            threadId = threadId,
     ...
     */
    private fun parseTurnDiffUpdated(params: JsonObject): UnifiedEvent? {
        val threadId = params.string("threadId") ?: return null
        val turnId = params.string("turnId") ?: return null
        val diff = params.string("diff") ?: return null
        return UnifiedEvent.TurnDiffUpdated(threadId = threadId, turnId = turnId, diff = diff)
    }
     ...
     */
    private fun parseTurnPlanUpdated(params: JsonObject): UnifiedEvent? {
        val turnId = params.string("turnId") ?: return null
        ...
        val steps = params.arrayValue("plan")
            ...
            ?.mapNotNull { value ->
                ...
                UnifiedRunningPlanStep(step = text, status = value.string("status").orEmpty())
            }
            .orEmpty()
        ...
        return UnifiedEvent.RunningPlanUpdated(
     ...
     */
    private fun parseThreadCompacted(params: JsonObject): UnifiedEvent? {
        ...
        return UnifiedEvent.ItemUpdated(item)
    }
     ...
     */
    private fun parseError(params: JsonObject): UnifiedEvent? {
        ...
        return UnifiedEvent.Error(message = message, terminal = !willRetry)
    }
}
