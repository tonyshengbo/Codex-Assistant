...
internal class CodexUnifiedItemTypeRouter(
    ...
    fun parse(
        ...
        return when (canonicalType) {
            CodexUnifiedItemTypeAliases.AGENT_MESSAGE -> typeParsers.parseAgentMessage(item = item, status = status)
            CodexUnifiedItemTypeAliases.REASONING -> typeParsers.parseReasoning(item = item, status = status)
            CodexUnifiedItemTypeAliases.COMMAND_EXECUTION -> typeParsers.parseCommandExecution(
                item = item,
                status = status,
                bufferedOutput = state.activityOutputBuffers[itemId]?.toString().orEmpty(),
            )
            CodexUnifiedItemTypeAliases.FILE_CHANGE -> typeParsers.parseFileChange(
                item = item,
                status = status,
                previousChanges = previous?.fileChanges.orEmpty(),
            )
            CodexUnifiedItemTypeAliases.CONTEXT_COMPACTION -> {
            ...
            CodexUnifiedItemTypeAliases.WEB_SEARCH -> typeParsers.parseWebSearch(item = item, status = status)
            CodexUnifiedItemTypeAliases.MCP_TOOL_CALL -> typeParsers.parseMcpToolCall(item = item, status = status)
            CodexUnifiedItemTypeAliases.PLAN -> typeParsers.parsePlan(item = item, status = status)
            CodexUnifiedItemTypeAliases.USER_MESSAGE -> typeParsers.parseUserMessage(item = item, status = status)
