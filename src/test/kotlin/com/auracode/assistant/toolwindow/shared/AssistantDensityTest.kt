...
class AssistantDensityTest {
    ...
    fun `compact ui tokens define reduced text and spacing scale`() {
        ...
        assertEquals(4.4.dp, tokens.spacing.xs)
        assertEquals(8.8.dp, tokens.spacing.sm)
        assertEquals(13.2.dp, tokens.spacing.md)
        assertEquals(17.6.dp, tokens.spacing.lg)
        assertEquals(15.4.sp, tokens.type.body)
        assertEquals(12.1.sp, tokens.type.meta)
        assertEquals(15.4.sp, tokens.type.sectionTitle)
        assertEquals(16.5.sp, tokens.type.title)
        assertEquals(26.4.dp, tokens.controls.headerActionTouch)
        assertEquals(19.8.dp, tokens.controls.iconLg)
        assertEquals(41.8.dp, tokens.controls.sendButton)
        assertEquals(44.dp, tokens.controls.railItem)
        assertEquals(48.4.dp, tokens.controls.attachmentCard)
    }
    ...
    fun `compact markdown tokens keep code and quote surfaces tighter than defaults`() {
        ...
        assertEquals(8.8.dp, tokens.markdown.codePadding)
        assertEquals(8.8.dp, tokens.markdown.quoteIndent)
        assertEquals(8.8.dp, tokens.markdown.tableCellPadding)
    }
