...
class InlineSubmissionInputBehaviorTest {
    @Test
    ...
    fun `subtle emphasis style uses weak accent highlight instead of filled button`() {
        ...
        val weak = submissionCardActionChrome(
            emphasized = true,
        ...
        val default = submissionCardActionChrome(
            emphasized = true,
            emphasisStyle = SubmissionCardActionEmphasisStyle.PRIMARY_FILL,
            enabled = true,
            danger = false,
            p = palette,
        )
    ...
    fun `composer interaction click focus prefers active input target`() {
        assertEquals(
            SubmissionInteractionFocusTarget.CARD,
            restoreSubmissionInteractionFocusTarget(preferInput = false),
        )
        assertEquals(
            SubmissionInteractionFocusTarget.INPUT,
            restoreSubmissionInteractionFocusTarget(preferInput = true),
        )
    }
